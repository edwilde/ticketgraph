...
describe("tickets.import_json", () => {
  it("dry_run returns counts and warnings without mutating DB", async () => {
    ...
    const result = await tool.handle(
      tool.parseArgs({ project: "demo", file: filePath, dry_run: true }),
    );
  ...
  it("live import inserts tickets, tags, and relations", async () => {
    ...
    const result = await tool.handle(
      tool.parseArgs({ project: "demo", file: filePath }),
    );
    ...
    const relations = db
      ...
      .all("demo") as Array<{ from_id: string; to_id: string; kind: string }>;
    ...
    const tags = db
      ...
      .all("demo", "T1") as Array<{ tag: string }>;
  ...
  it("_created audit row is back-dated to ticket created_at", async () => {
    ...
    await tool.handle(tool.parseArgs({ project: "demo", file: filePath }));
  ...
  it("duplicate without force aborts with McpError", async () => {
    ...
    await expect(
      tool.handle(tool.parseArgs({ project: "demo", file: filePath })),
  ...
  it("duplicate with force overwrites cleanly", async () => {
    ...
    const result = await tool.handle(
      tool.parseArgs({ project: "demo", file: filePath, force: true }),
    );
    expect(result.imported).toBe(true);
  ...
  it("missing fields default per §5 (status→open, type→task, description→'')", async () => {
    ...
    const ticket = db
      ...
      .get("demo", "T1") as { status: string; type: string; description: string };
  ...
  it("parseArgs throws McpError when file is missing", () => {
    const { db, tool } = setup();
    expect(() => tool.parseArgs({ project: "demo" })).toThrow(McpError);
    db.close();
  });
  ...
  it("closed_at is preserved from file when present", async () => {
    ...
    const ticket = db
      ...
      .get("demo", "T1") as { closed_at: string | null };
