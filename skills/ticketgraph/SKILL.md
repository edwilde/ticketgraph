...
Mutating commands (`add`, `add_many`, `update`, `link`, `unlink`, `set_parent`, `append_to_description`, `add_tag`, `remove_tag`, `import_json`, `export`) exist too. Each change writes an audit entry. Confirm exact flags with `<command> --help` before running — e.g. `ticketgraph add --title "..." --type bug`, `ticketgraph update T5 --status done`. Use `add_many` to create many tickets in one transaction via `--json '<obj>'` (or `--json -` for stdin); `--project <id>` can be given as a normal flag alongside it, and should be whenever the target project is known.
