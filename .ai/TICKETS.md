...
**Second trace — the empty-board case (2026-06-04, verified live):** a project where 152/155 tickets are done and the only 3 left are `blocked`/`deferred` (nothing open or in-progress). New failure modes the first trace didn't surface:
1. **`next` returns blank with no explanation.** `ticketgraph next` printed `ticket -` / `reason -` because nothing is open. The model can't distinguish "board is clear" from "tool returned nothing / I called it wrong" — there's no `reason` like "no open tickets (3 blocked, 1 deferred — see `list --status all`)". Empty `next` should explain *why* it's empty and point at the non-open work.
2. **No single command answers "is there anything outstanding at all?"** The model resorted to `list --status all | grep -vi done` (fragile — greps the word "done") and then a `for s in blocked deferred in_progress open` loop, one `list` per status, to reconstruct the board. There's no "everything not-done" view; the default filter hides `deferred`, and `--status all` includes the 152 done rows.
3. **`--ids` with multiple values still mis-parses.** `get --ids T143 T86-followup T48` failed with `get takes at most one positional argument (got 2)` — the array flag consumed only the first value and the rest fell through to positionals. So even the *documented* plural-flag multi-fetch path is broken, not just repeated `--id` (trace-1 item 4). Forced yet another per-id loop.
4. **`get` body still invisible** in `table`/`compact` (same P1 as trace-1 item 2) → final fallback to N× `get --format json`.
...
- **(P1) Multi-id via CLI must never silently drop ids.** Covers BOTH paths: repeated `--id` (trace-1, silently last-wins) AND `--ids a b c` (trace-2: the array flag took only the first value, the rest fell through to positionals → `got 2` error). Pick one coherent rule: make repeated `--id` collect into `ids`, accept `--ids a,b,c` (comma-split) and/or `--ids a b c` (consume the run), or **throw** loudly. Returning/erroring on a partial id set is the footgun. Prefer: accept the multi-value form as the array (most ergonomic for the model) with a clear error if it can't.
- **(P1) Empty `next` must explain *why* it's empty.** When nothing is open/in-progress, `next` returns `ticket -` / `reason -` — indistinguishable from a tool error or a bad call (trace-2 item 1). It should return a `reason` that says the board has no open work AND points at the non-open counts, e.g. `"no open tickets — 3 blocked, 1 deferred; run \`list --status all\`"`. The reason field already exists for the populated case; populate it for the empty case too.
- **(P2) A single "is there anything outstanding?" view.** Trace-2 reconstructed the board with `list --status all | grep -vi done` (fragile) then a per-status loop, because the default filter hides `deferred` and `--status all` drowns it in 152 done rows. Provide a "not-done" view: a `--status outstanding` alias (= open/in_progress/blocked/deferred — everything not done/closed) and/or have `stats` lead with the not-done breakdown. One call should answer "is the board clear?".
- **(P2) Make the one-call path discoverable.** `list --include_description` already answers the whole question in ONE call for non-deferred tickets — it's just unknown. Document it in `skills/ticketgraph/SKILL.md` and top-level `--help`. Consider a `--status outstanding` alias (see above) and a help line stating `deferred`/`done` are excluded by default.
...
- Passing multiple ids via the CLI returns all of them (or fails loudly) — covered by a CLI spawn test for BOTH repeated `--id` and `--ids a b c`; neither silently drops ids.
- `next` on a board with no open tickets returns a non-empty `reason` naming the not-done counts (blocked/deferred) and the command to see them — asserted on a seeded all-blocked/deferred fixture.
- One command answers "is anything outstanding?" without a per-status loop: `--status outstanding` (or equivalent) returns the not-done set in a single call — covered by a test.
...
**Notes:** four-stage pipeline. Relates to T20 (response shapes) and T24 (output formatting). The P1 set is the bulk of the win — `get` body in compact, multi-id (both flag forms), and a non-empty `next` reason on a clear board; the rest is discoverability polish. Two real traces now back this ticket: the original 4-ticket read-path session (2026-06-03) and the empty-board session (2026-06-04). Effort **3**: formatter change is localised, the multi-id/empty-`next` fixes are flag-/handler-layer tweaks, docs are cheap.
...
**Why (user-requested, 2026-06-16):** the `outstanding` skill (`/ticketgraph:outstanding`; shipped v0.10.0 as `outstanding-tickets`, renamed v0.11.0) is the first thin "do the minimum, then stop" command-wrapper skill over the CLI — it saves tokens on a common question *and* dogfoods the token-cheap pitch. The repo already ships a plugin manifest + the `ticketgraph` reference skill, but there's no deliberate set of one-shot command wrappers and no curation of which CLI verbs earn one. This ticket builds that family out and confirms the plugin packaging surfaces them.
**Scope:**
- Decide the wrapper family. Name each skill tersely so the plugin namespace carries the domain — `/ticketgraph:<verb>`, not `/ticketgraph:<verb>-tickets`. Candidates (each must stay one-or-two calls and *stop*, per the `outstanding` discipline): `next`, `get`, `find` (`search`), `blocked` (`list --status blocked`), `stats`. Pick the few that are genuinely common; skip verbs that don't benefit from a wrapper.
- Each wrapper = its own tiny `skills/<name>/SKILL.md`, auto-discovered, consistent in shape with `outstanding` (one command, present, stop; explicit "do NOT pile on other calls").
...
**Progress (2026-06-16, v0.11.0):** the *installable* piece landed — added `.claude-plugin/marketplace.json` (repo doubles as its own single-plugin marketplace, `source: "./"`, no `version` so `plugin.json` stays the single source) + install docs in README/`docs/install.md`. Confirmed via claude-code-guide against the current plugin-marketplace docs. **Caveat documented:** `/plugin install` delivers skills only; the `ticketgraph` binary still needs a per-platform build (native `better-sqlite3`), so npm publish remains the real binary-distribution path (deferred — user chose marketplace-only for now). Also renamed the skill `outstanding-tickets` → `outstanding` so the plugin namespace reads cleanly (`/ticketgraph:outstanding`, not the redundant `…:outstanding-tickets`) — establishing the terse-name convention for the rest of the family. **Still open:** ≥1 more wrapper skill (only `outstanding` so far; acceptance wants ≥2) and the npm-publish decision.
