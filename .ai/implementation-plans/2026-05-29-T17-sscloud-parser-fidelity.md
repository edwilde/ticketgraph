...
**Goal:** Improve the sscloud parser's fidelity on two high-value, high-precision axes — infer `type=spike` from `Spike:` titles, and emit `follows_up` relations from explicit shipped-note phrasings ("follow-up tickets filed: T…", "Spawned T…-T… follow-ups"). Two other proposed axes are deliberately **won't-do** (documented below).
**Architecture:** Pure-function changes to `src/parsers/sscloud.ts` only; new committed fixtures + tests in `tests/fixtures/sscloud/` and `src/parsers/sscloud.test.ts`. No tool/server changes, no schema changes.
...
- **The live sscloud `TICKETS.md` is already migrated and DELETED** (a prior session ran the live import per spec §7 and removed the source). So: (a) these improvements can't be re-calibrated against the live file; (b) they benefit a *future* re-import (the source would have to be recovered from the sscloud repo's git history, then `import_json({ force: true })`) or other sscloud-format projects; (c) **all testing is against the committed fixtures in `tests/fixtures/sscloud/`** — which is the version-controlled contract anyway (spec §16).
...
**Files:**
- Modify: `src/parsers/sscloud.ts`
- Create: `tests/fixtures/sscloud/21-spike-type.md`
- Modify: `src/parsers/sscloud.test.ts`

**Decisions:**
- After parsing the `title`, set `type = "spike"` when the title matches `/^Spike\b/i` or starts with `Spike:` (the real sscloud T3 was "Spike: verify deploy log poll endpoint shape"). Otherwise leave `type` unset (defaults to `task` per import-format / §5).
- Only this one high-precision signal. Do NOT attempt `bug` inference (no reliable keyword in sscloud's `T<n>` tickets).
...
**Verify:** `npm test src/parsers/sscloud.test.ts` green; the spike fixture asserts `type: "spike"`.
...
**Files:**
- Modify: `src/parsers/sscloud.ts`
- Modify: `src/parsers/sscloud.test.ts` (use existing fixtures 16 + 20; add assertions)
...
**Files:**
- Modify: `src/parsers/sscloud.ts` (a short comment block documenting the won't-do decisions for items 1 + 3, so a future reader knows they were considered and why rejected).
...
**Verify:**
1. `npm run build` → exit 0 (parser still builds to `dist/parsers/sscloud.js`).
...
6. Sanity: run the CLI on fixture 16 (`node dist/parsers/sscloud.js tests/fixtures/sscloud/16-umbrella-spawned.md`) and confirm the JSON now contains the `follows_up` relations.

---

## Caveats & known risks

- **Live sscloud already migrated + deleted**: these improvements do not retroactively fix the already-imported data. To benefit, recover the source from the sscloud repo's git history and `import_json({ force: true })`, or apply to other sscloud-format projects. The improvements are still correct and worth having in the parser.
...
- **Live-file caveat**: sscloud was already migrated + the source deleted, so these gains apply to a future re-import (recover source from git history → `import_json({force:true})`) or other sscloud-format projects. Tests are fixture-based (the version-controlled contract).
