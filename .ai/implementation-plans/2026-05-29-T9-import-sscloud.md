# T9 — Migration tool (`tickets.import_json`) + sscloud parser

> **For the implementer:** Use `subagent-driven-development` to implement task-by-task.

**Goal:** Ship the generic JSON-intermediate importer (`tickets.import_json`) and the sscloud-specific parser that produces that intermediate, so sscloud's 4,127-line `TICKETS.md` migrates into the store with ≥95% fidelity.
**Architecture:** A shared TypeScript type for the JSON intermediate (`src/lib/import-format.ts`). The `tickets.import_json` MCP tool consumes it (3-pass transactional write, dry-run, force). A pure `parseSscloud(md): ImportFile` function in `src/parsers/sscloud.ts` plus a thin CLI entry. Format doc in `docs/import-format.md`. Tests use version-controlled fixtures (spec §16), never the live file.
...
- **Two halves**: (a) the generic `tickets.import_json` ingester — project-agnostic; (b) the sscloud parser — format-specific. They meet at the JSON intermediate (spec §7).
...
- **sscloud parser heuristics** (spec §7, calibrated against the real `~/Scripts/sscloud/.ai/TICKETS.md`):
  - `### T<n> — <title>` heading → `id` (`T<n>`) + `title`. Titles may contain backticks/punctuation — take everything after the em-dash `— `.
  - `**Status:**` line: presence of `✅ Done` (or `Done`) → `status=done`. Absence of a Status line → `status=open` (most open tickets in sscloud just omit it). Other markers if present: `In progress` → in_progress; `Deferred` → deferred; `Blocked` → blocked.
  ...
  - `created_by` → `"migrated:sscloud"` (spec §7).
  - Commit refs (`commit 20d91af`) → preserved verbatim in `description`, NOT a structured field.
- **Parser is a PURE function** `parseSscloud(md: string): ImportFile` — no file I/O inside. The CLI wrapper reads stdin/argv and writes stdout. This keeps it unit-testable against fixture strings.
...
- **Tests use fixtures, NEVER the live file** (spec §16): `tests/fixtures/sscloud/*.md` holding the 20 most-distinctive ticket shapes. The ≥95% round-trip acceptance is measured by running the parser over the real file via the CLI (a manual/acceptance step), but the committed tests run on fixtures.
...
## Task 3: sscloud parser — core (`parseSscloud`)

**Files:**
- Create: `src/parsers/sscloud.ts`
- Create: `tests/fixtures/sscloud/*.md` (≥20 distinctive shapes)
- Create: `src/parsers/sscloud.test.ts`

**Decisions:**
- Export pure `parseSscloud(md: string): ImportFile` with `project_id: "sscloud"`.
...
## Task 4: sscloud parser — CLI entry

**Files:**
- Modify: `tsup.config.ts` (add `src/parsers/sscloud.ts` as a second entry so it builds to `dist/parsers/sscloud.js`)
- Modify: `src/parsers/sscloud.ts` (add a CLI guard)

**Decisions:**
- CLI: `node dist/parsers/sscloud.js <input.md> [--report]` reads the file, runs `parseSscloud`, writes JSON to stdout. `--report` writes a parse summary (ticket count, relation count by kind, skipped/ambiguous lines) to stderr.
...
- tsup: change `entry` to `["src/server.ts", "src/parsers/sscloud.ts"]` so both build. Confirm `dist/parsers/sscloud.js` is produced and is executable-ish (it's run via `node`, no shebang needed, but harmless to add one).

**Don't:**
- Don't make the CLI the only interface — `parseSscloud` stays independently importable/testable.
...
**Verify:** `npm run build` produces `dist/parsers/sscloud.js`. Running it on a fixture file emits valid JSON parseable by `validateImportFile`.
...
- The ≥95% round-trip acceptance against the REAL sscloud file is a MANUAL step (not a committed test, since tests can't read the live file). Document the command in `docs/import-format.md`: `node dist/parsers/sscloud.js ~/Scripts/sscloud/.ai/TICKETS.md --report > /tmp/sscloud.json` then eyeball + `tickets.import_json({ dry_run: true })`.

**Don't:**
- Don't add a committed test that reads the live sscloud file.
...
**Decisions:**
- Run the parser over the real `~/Scripts/sscloud/.ai/TICKETS.md`, capture the `--report` output, and record the round-trip fidelity (parsed tickets / 133 headings) in the review record. Target ≥95% (≥127 of 133).
...
**Verify:**
1. `npm run build` → exit 0; `dist/server.js` + `dist/parsers/sscloud.js` both exist.
2. `npm test` → exit 0; all green.
3. `npm run typecheck` → exit 0.
4. `node dist/server.js --help` → exit 0.
5. `grep -rn 'console\.' src/ tests/` → 0 hits **EXCEPT** the sscloud CLI entry, which legitimately writes to stdout/stderr. **Decision:** the CLI must use `process.stdout.write` / `process.stderr.write`, NOT `console.*`, to keep the grep invariant intact. (The server's stdout-sacredness doesn't apply to a standalone CLI, but keeping the zero-console rule uniform avoids a special case.)

---

## Caveats & known risks

- **sscloud format drift**: the parser is calibrated to the file as it exists today. The live file changes; that's exactly why tests use frozen fixtures (spec §16). The ≥95% metric is a point-in-time acceptance, not a standing test.
...
- **Second tsup entry**: adding `src/parsers/sscloud.ts` must not regress the server bundle or its shebang. Verify both outputs after the config change.
...
- `npm run build` → exit 0; `dist/server.js` + `dist/parsers/sscloud.js` both produced; shebang intact.
...
- Calibration re-run on real sscloud file confirmed: **133/133 headings parsed (100%)**, relations blocks=113, follows_up=1, supersedes=1, 0 skipped.
...
| 2 | sscloud heading regex broadened to `T\d+[\w-]*` for 13 real suffixed ids (T50a, T49-followup) | Deviation | Approved — within "calibrated against the real file" intent; 120 base + 13 suffixed = 133 |
...
- **Parser purity**: `parseSscloud(md)` is pure string→ImportFile; the CLI (`dist/parsers/sscloud.js`, second tsup entry) is the only I/O, guarded by an `import.meta.url` check and using `process.stdout/stderr.write` (not console) to keep the repo-wide zero-console invariant.
...
- Live sscloud import into the real DB (deliberate separate step).
