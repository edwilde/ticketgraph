...
- **Numbering scheme inference (spec §6 / T5 Scope):** scan `tickets.id` for the project. Extract the prefix-and-number pattern via regex (`^([A-Z]+(?:-)?)(\d+)$`). If a single prefix dominates (>50% of tickets), use it with `max(n)+1`. If multiple co-exist (e.g. wesabe's `BUG-`/`FEAT-`), error and require explicit `id`. Empty project → `T1`.
...
- **Numbering scheme inference can be subjective** — if a sscloud-style project later starts using `BUG-` tickets, the inference will start failing once `BUG-` crosses 50% of new rows. T7's `update` (or future cleanups) may need to expose this knob. For now: deterministic + opinionated, with explicit `id` as the escape valve.
