...
- **Command-name rule:** CLI command = tool name minus the `tickets.` prefix, **underscores preserved** (`tickets.add_many`→`add_many`, `tickets.register_project`→`register_project`). One mechanical rule, no `_`→`-` prettification (optimise for Claude's accuracy — the author's standing preference).
