...
**Goal:** Add `src/parsers/wesabe.ts` producing the same JSON intermediate `tickets.import_json` already consumes, so wesabe's 3,606-line / 351-ticket `TICKETS.md` migrates with ≥95% fidelity.
**Architecture:** Reuse everything from T9 — `ImportFile` types, `validateImportFile`, `tickets.import_json` (UNCHANGED). Only add a wesabe-specific pure parser + CLI entry (third tsup entry) + fixtures + tests.
...
- **import_json is done and untouched** (T9). T10 only produces a different-shaped JSON intermediate from a different source format. The parser mirrors `src/parsers/sscloud.ts`'s structure (pure function + guarded CLI).
- **wesabe format** (calibrated against the real `~/sites/wesabe/.ai/TICKETS.md`, 351 `### NAMESPACE-NN:` headings):
  ...
  - `created_by = "migrated:wesabe"`, `project_id = "wesabe"`.
  - `closed_at`: wesabe rarely carries inline ship dates; if a parseable date appears in the body for a done ticket, normalise it; else null (legal §7).
- **Blocked-by direction is the inverse of "this lists its blockers"**: the heading ticket lists what blocks IT, so each extracted ref becomes `from = ref (blocker)`, `to = headingId (blocked)`. Double-check against the sscloud parser's convention so both are consistent (sscloud's Blockers line is the same semantics — verify sscloud did `from = ref, to = ticket`; match it).
- **Two-pass**: tickets first (all ids exist), relations second.
- **Tests use fixtures** `tests/fixtures/wesabe/*.md` (≥10 distinctive shapes), NEVER the live file (spec §16).
- **Round-trip ≥95%** measured manually against the real file via the CLI `--report` path; not a committed test.

---

## Task 1: `src/parsers/wesabe.ts` — core parser

**Files:**
- Create: `src/parsers/wesabe.ts`
- Create: `tests/fixtures/wesabe/*.md` (≥10 shapes)
- Create: `src/parsers/wesabe.test.ts`

**Decisions:**
- Export pure `parseWesabe(md: string): ImportFile` (project_id "wesabe"). Reuse `ImportFile`/`ImportTicket`/`ImportRelation` from `src/lib/import-format.ts`.
...
**Don't:**
- Don't do file I/O in `parseWesabe` — pure string→object.
- Don't change `import_json` or `import-format.ts`.
- Don't read the live `~/sites/wesabe/.ai/TICKETS.md` in any test.
...
## Task 2: wesabe CLI entry + tsup

**Files:**
- Modify: `tsup.config.ts` (add `src/parsers/wesabe.ts` → third entry)
- Modify: `src/parsers/wesabe.ts` (CLI guard, mirroring sscloud's)

**Decisions:**
- CLI: `node dist/parsers/wesabe.js <input.md> [--report]` → JSON to stdout via `process.stdout.write`; report (ticket count, relations by kind, namespace histogram, skipped lines) to stderr. Guard with the `import.meta.url` check.
- tsup entry array becomes `["src/server.ts", "src/parsers/sscloud.ts", "src/parsers/wesabe.ts"]`. Confirm all three build; server shebang intact.

**Don't:**
- Don't use `console.*` — `process.stdout/stderr.write` only (repo-wide invariant).
- Don't regress the server or sscloud builds.

**Implement:** CLI + build config.

**Verify:** `npm run build` produces `dist/parsers/wesabe.js`. Running it on a fixture emits JSON that `validateImportFile` accepts.
...
**Decisions:**
- Run `node dist/parsers/wesabe.js ~/sites/wesabe/.ai/TICKETS.md --report`. Record: headings parsed / 351 (target ≥95% = ≥334), relations by kind, namespace histogram (sanity-check the type/tag mapping). Do NOT live-import into `~/.claude/tickets.db`.
...
**Verify:**
1. `npm run build` → exit 0; `dist/server.js` + `dist/parsers/sscloud.js` + `dist/parsers/wesabe.js` all exist; server shebang intact.
...
- **Multi-prefix ids by design**: wesabe ids are inherently namespaced; `tickets.add`'s numbering inference would refuse auto-id on this project (multiple prefixes) — which is correct; imported ids are explicit, and any future manual add must pass an explicit id.
- **Blocked-by direction consistency**: both parsers must emit `from = blocker-ref, to = this-ticket` for `blocks`. Verify the sscloud parser did the same before shipping (a mismatch would make `blockers_of` wrong for one project).
...
- **Format drift**: same as sscloud — frozen fixtures for tests, point-in-time ≥95% acceptance against the live file.

---

## Validation review

(none — mirrors the T9 sscloud parser against an established, unchanged import path. The only new judgement is the namespace→type/tag mapping, documented above.)
...
- `npm run build` → exit 0; all 3 dist entries (server + sscloud + wesabe) present with shebangs.
...
- Calibration re-run on real wesabe file: **351/351 headings (100%)**, 104 blocks relations, 0 skipped. Namespace histogram sane (UX:144, DESIGN:45, FE:28, BUG:24, ...).

### Triage Summary
| # | Finding | Type | Decision |
|---|---------|------|----------|
| 1 | All 18 wesabe parser criteria (pure fn, heading/status/epic/blockquote/AC, blocked-by direction matching sscloud, namespace type+tag, two-pass, fixture-only tests) | Completed as planned | Verified |
...
- **wesabe parser mirrors sscloud's structure** (pure fn + `import.meta.url`-guarded CLI, `process.stdout/stderr.write`). Blocked-by direction is identical: `{ from: blockerRef, to: headingId, kind: "blocks" }`.
...
- Live wesabe import into the real DB (deliberate separate step, spec §7).
