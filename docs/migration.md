...
**ticketgraph becomes the canonical store.** Once you migrate, the MCP database is the source of truth for ticket state. TICKETS.md is not regenerated — ticketgraph does not have a markdown export pipeline, by design (see §2 of the [design spec](specs/2026-05-28-ticketgraph-design.md)).
...
```sh
node dist/parsers/sscloud.js /path/to/project/.ai/TICKETS.md --report > /tmp/myproject.json
```

### wesabe format

```sh
node dist/parsers/wesabe.js /path/to/project/.ai/TICKETS.md --report > /tmp/myproject.json
```
